	...
	function paginate($object = null, $scope = array(), $whitelist = array()) {
		...
		if (method_exists($object, 'paginateCount')) {
			...
			if(isset($extra['group'])){
				if(!is_array($extra['group']))
					$extra['group'][] = $extra['group'];

				$unhavedextra['group'] = $extra['group'];
				foreach($extra['group'] as $i => $extr){
					if(strpos(low($extr),'having count')!==false){
						$having = true;
						unset($unhavedextra['group'][$i]);
					}
				}
			}

			if($having){
				$countresults = $object->find('all', array_merge($parameters, $extra));
				$count = sizeof($countresults);
			} else {
				$count = $object->find('count', array_merge($parameters, $unhavedextra));
			}
		}
